frappe.sms_alert = {
	setup_fieldname_select: function(frm) {
		...
		frappe.model.with_doctype(frm.doc.document_type, function() {
			...
			console.log("trace you log")
			console.trace(phone_fields)
			frappe.meta.get_docfield("SMS Alert Recipient", "sms_by_document_field",
				// set first option as blank to allow sms alert not to be defaulted to the owner
				frm.doc.name).options = [""].concat(["owner"].concat(phone_fields));
...
frappe.ui.form.on("SMS Alert", {
	...
	refresh: function(frm) {
		frappe.sms_alert.setup_fieldname_select(frm);
	...
	event: function(frm) {
		if(in_list(['Days Before', 'Days After'], frm.doc.event)) {
			frm.add_custom_button(__('Get Alerts for Today'), function() {
				frappe.call({
					method: 'notification.notification.doctype.sms_alert.sms_alert.get_documents_for_today',
					args: {
						sms_alert: frm.doc.name
					},
					callback: function(r) {
						if(r.message) {
