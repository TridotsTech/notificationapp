...
from notification.notification.api import send_sms_to_receivers

class SMSCenterTool(Document):
	@frappe.whitelist()
	def create_receiver_list(self):
		rec, where_clause = '', ''
		if self.send_to == 'All Customers':
			try:
				nst = frappe.new_doc('Customers')
				ref_doctype ="Customers"
			except Exception:
				ref_doctype ="Customer"
			if self.customer:
				where_clause = " and dl.name = '%s'" % \
					self.customer.replace("'", "\'") or " and ifnull(dl.link_name, '') != ''"

		if self.send_to:
			if ref_doctype =="Customer":
				rec = frappe.db.sql("""select c.customer_name,
				c.mobile_no from `tab{ref_doctype}` c, `tabDynamic Link` dl  where ifnull(c.mobile_no,'')!='' and
				c.docstatus != 2 and dl.parent = c.name {condition}""".format(ref_doctype=ref_doctype, condition= where_clause))
			else:
				rec = frappe.db.sql("""select dl.full_name,
					dl.phone from `tab{ref_doctype}` dl  where ifnull(dl.phone,'')!='' {condition}""".format(ref_doctype=ref_doctype, condition=where_clause))
		
		# 	rec = frappe.db.sql("""select CONCAT(ifnull(dl.first_name,''), ' ', ifnull(dl.last_name,'')),
		# 		dl.phone from `tab{ref_doctype}` dl  where ifnull(dl.phone,'')!='' {condition}""".format(ref_doctype=ref_doctype, condition=where_clause))
	...
	def send_sms(self):
		...
		if receiver_list:
			# response = send_sms(receiver_list, cstr(self.message))
			response = send_sms_to_receivers(receiver_list, cstr(self.message))
			self.message = ""
			self.send_to = ""
			self.receiver_list = ""
